...
cdef extern from "tsne.h":
    cdef cppclass TSNE:
        TSNE()
        void run(double* X, int N, int D, double* Y, int no_dims, double perplexity, double theta, int rand_seed, bool skip_random_init, double *init, bool use_init)
...
cdef class BH_SNE:
    ...
    def run(self, X, N, D, d, perplexity, theta, seed, init, use_init):
        ...
        self.thisptr.run(&_X[0,0], N, D, &Y[0,0], d, perplexity, theta, seed, False, &_init[0,0], use_init)
        return Y
